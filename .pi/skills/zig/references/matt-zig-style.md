...
**Ring-native ops need own address builders:** std's address types (`net.IpAddress`, `net.UnixAddress`) are Io-vtable-bound — their listen/connect are blocking `Io` paths and the raw sockaddr bytes are never exposed. A ring op (`IoUring.connect` etc.) needs the sockaddr pointer + addrlen itself, so the sockaddr builder for any ring-bound endpoint is handoff code (UnixTarget's category). Validate std first every time — but if the piece is ring-native, owning it is correct, not duplication. (Owner-raised 2026-09-16; UnixAddress checked and declined: thin path-slice wrapper, blocking-only, hard-coded 108 where our derived capacity check is stricter.)
