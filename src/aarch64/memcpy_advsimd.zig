...
comptime {
    if (enabled) {
        @export(&copyEntry, .{ .name = "fastmem_advsimd_copy", .visibility = .hidden });
        @export(&copyEntry, .{ .name = "fastmem_advsimd_move", .visibility = .hidden });
    }
}

pub fn copyEntry() align(64) callconv(.naked) void {
    asm volatile (
        \\hint 34
        \\    add    x4, x1, x2
        \\    cmp    x2, 128
